...
const editTaxTypeRef = useRef(null);
const editTaxNameRef = useRef(null);
const editInputTaxRef = useRef(null);
const editOutputTaxRef = useRef(null);
const editBranchRef = useRef(null);
                      ...
                      useEffect(() => {
                        const fetchItemTypes = async () => {
                          try {
                            const response = await ItemTypeModel.getItemTypes(user_id, user_types,1000,1,"","True"); // Make sure user_id & user_types are defined
                      ...
                      useEffect(() => {
                        const fetchBranches = async () => {
                          try {
                            const response = await settingsTaxModel.getBranches(user_id,'',1000,1,'','True');
                      ...
                      const getBranchName = (id) => {
                        ...
                        const handleSubmitTax = async () => {
                          ...
                          if (Object.keys(validationErrors).length > 0) {
                            ...
                            if (validationErrors.item_type) itemTypeRef.current?.focus();
                            else if (validationErrors.tax_type) taxTypeRef.current?.focus();
                            else if (validationErrors.tax_name) taxNameRef.current?.focus();
                             else if (validationErrors.branch) branchRef.current?.focus();
                            else if (validationErrors.input_tax) inputTaxRef.current?.focus();
                            else if (validationErrors.output_tax) outputTaxRef.current?.focus();
                            ...
                            if (response.status === 201 || response.status === 200) {
                              ...
                              ...error.response.data.errors,
                                                    ...
                                                    const handleEditSubmitTax = async () => {
                                                     ...
                                                     if (!validateEditTax()) {
                                                      if (editTaxErrors.item_type) editItemTypeRef.current?.focus();
                                                      else if (editTaxErrors.tax_type) editTaxTypeRef.current?.focus();
                                                      else if (editTaxErrors.tax_name) editTaxNameRef.current?.focus();
                                                      else if (editTaxErrors.input_tax) editInputTaxRef.current?.focus();
                                                      else if (editTaxErrors.output_tax) editOutputTaxRef.current?.focus();
                                                      else if (editTaxErrors.branch) editBranchRef.current?.focus();
                                                      return;
                                                    }
                                  ...
                                  <div className="bg-white rounded-xl shadow-md  w-[90vw] max-w-[500px] h-[90vh] max-h-[600px] flex flex-col overflow-y-auto gap-4"style={{padding:'20px'}}> 
                                    ...
                                    <div>
                                      ...
                                      <input type="text" 
                                        ...
                                        name="tax_name"
                                      />
                                      <p className="text-xs text-red-400">{errors.tax_name || errors.name}</p>
                                      ...
                                      <div className="w-full flex flex-col gap-2">
                                        ...
                                        >
                                          <option className="text-xs text-gray-400"  value="">--Select Branch--</option>
                                          {branches.map((branch) => (
