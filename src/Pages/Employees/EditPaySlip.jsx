...
const user_id = auth?.login_id || null;
const userTypes = auth?.can_manage_user_types || {};
const user_types = Object.keys(userTypes).join(',');
...
useEffect(() => {
  if (payslip) {
    ...
    const loadPayslip = async () => {
      try {
        const res = await PayslipModel.getPayslip(user_id, user_types, 100, 1, search, status);
        const allPayslips = res?.data?.data || [];

        const matchedPayslip = allPayslips.find((p) => String(p.id) === String(id));
        if (matchedPayslip) {
          setSelectedPayslip(matchedPayslip);
        } else {
...
useEffect(() => {
  ...
  const fetchPositions = async () => {
    try {
      const res = await employeePositionModel.getPositions(user_id);
      if (isMounted) setPositions(res?.data?.data || []);
    } catch (error) {
      if (isMounted) console.error("Error fetching positions:", error);
    }
  };
...
useEffect(() => {
  ...
  const fetchDepartments = async () => {
    try {
      const res = await employeeDepartmentModel.getDepartments(user_id);
      if (isMounted) setDepartments(res?.data?.data || []);
    } catch (error) {
      if (isMounted) console.error("Error fetching departments:", error);
    }
  };
...
useEffect(() => {
  ...
  const fetchEmployees = async () => {
    try {
      const res = await employeeModel.getEmployees(user_id);
      if (isMounted) setEmployees(res?.data?.data || []);
    } catch (error) {
      if (isMounted) console.error("Error fetching employees:", error);
    }
  };
...
useEffect(() => {
  if (selectedPayslip) {
    setFormData({
      ...
      salary_month: selectedPayslip.salary_month || '',
      payment_date: selectedPayslip.payment_date || '',

      // Earnings
      basic_salary: selectedPayslip.basic_salary || '0.00',
      hra: selectedPayslip.hra || '0.00',
      conveyance_allowance: selectedPayslip.conveyance_allowance || '0.00',
      medical_allowance: selectedPayslip.medical_allowance || '0.00',
      special_allowance: selectedPayslip.special_allowance || '0.00',
      other_allowance: selectedPayslip.other_allowance || '0.00',
      bonus: selectedPayslip.bonus || '0.00',
      overtime_pay: selectedPayslip.overtime_pay || '0.00',
      other_earnings: selectedPayslip.other_earnings || '0.00',

      // Deductions
      provident_fund: selectedPayslip.provident_fund || '0.00',
      esi: selectedPayslip.esi || '0.00',
      professional_tax: selectedPayslip.professional_tax || '0.00',
      income_tax: selectedPayslip.income_tax || '0.00',
      loan_recovery: selectedPayslip.loan_recovery || '0.00',
      other_deductions: selectedPayslip.other_deductions || '0.00',

      // Summary
      gross_earnings: selectedPayslip.gross_earnings || '0.00',
      total_deductions: selectedPayslip.total_deductions || '0.00',
      net_salary: selectedPayslip.net_salary || '0.00',
      gross_salary: selectedPayslip.gross_salary || '0.00',

      // Payment Info
      payment_mode: selectedPayslip.payment_mode || null,
      bank_account: selectedPayslip.bank_account || '',
      upi_id: selectedPayslip.upi_id || '',
      supporting_documents: null, // can't prefill file input

      remarks: selectedPayslip.remarks || '',
      is_paid: selectedPayslip.is_paid || false,
    });
  }
}, [selectedPayslip]);
                        ...
                        <div className="bg-white rounded-lg shadow-sm border text-gray-500 text-[12px] border-gray-100" style={{ padding: '20px', marginBottom: '10px' }}>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                <div>
                                    ...
                                    <input type="text"  value={getValueFromId(formData.employee,employees)} readOnly className="w-full input input-xs border border-gray-300 rounded-sm bg-gray-50" style={{ padding: '5px' }} />
                                </div>
                                <div>
                                    <label className="block text-[11px] font-medium text-gray-500">Department</label>
                                    <input type="text"  value ={ getValueFromId(formData.department,departments)} readOnly className="w-full border border-gray-300 input input-xs rounded-sm bg-gray-50" style={{ padding: '5px' }} />
                                </div>
                                <div>
                                    <label className="block text-[11px] font-medium text-gray-500 mb-1">Designation</label>
                                    <input type="text" value = {getValueFromId(formData.position,positions)}  readOnly className="w-full input input-xs rounded-sm border border-gray-300 rounded-sm bg-gray-50" style={{ padding: '5px' }} />
                                </div>
                             <div>
